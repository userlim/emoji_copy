...
export default function CategoryNav({ current }: { current?: string }) {
  return (
    <div className="flex flex-wrap gap-1.5 mb-6">
      <a href="/" className={`shrink-0 px-3 py-1.5 rounded-full text-sm font-semibold transition-colors ${!current ? 'text-white' : 'text-gray-400 bg-white/[0.04] hover:bg-gray-200'}`}
        style={!current ? { background: 'var(--accent)' } : {}}>🏠 Home</a>
      {cats.map(c => (
        <a key={c.href} href={c.href}
          className={`shrink-0 px-3 py-1.5 rounded-full text-sm font-semibold transition-colors ${current === c.href ? 'text-white' : 'text-gray-400 bg-white/[0.04] hover:bg-gray-200'}`}
